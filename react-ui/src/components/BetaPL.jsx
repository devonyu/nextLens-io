...
export default class BetaPL extends Component {
  ...
  getPics = () => {
    ...
    const shuffled = evenlyDistributedImages(allCategories);
    // cut for the first 15 images for now
    const trimmed = shuffled.slice(0, 15);
    this.setState({
      imgs: trimmed,
  ...
  handleOption = async option => {
    ...
    await axios({
      ...
      .then(({ data }) => {
        console.log(
          `Adding user id=${this.props.userInfo.id} photoid=${this.state.currentIndex} data=${data}`
        );
        if (affinity === true) {
    ...
    return (
      <Container fluid textAlign="center" onKeyUp={this.handleKeyDown}>
        ...
        <Progress
          ...
          percent={Math.round((this.props.likeProgress / 30) * 100)}
          progress
        />
        <SwipeableViews
          enableMouseEvents
          action={() => {
            console.log(
              'loaded, can show example to show use how to like and dislike!, load gif in a modal?'
            );
          }}
          onSwitching={() => {
          ...
          onChangeIndex={(index, lastIndex) => {
            console.log(`Swiped to ${index} from ${lastIndex}`);
          }}
        >
          {this.state.imgs.map((image, idx) => (
            <div key={idx}>{image ? <Image id="splashImage" src={image.urls.regular} /> : ''}</div>
          ))}
        </SwipeableViews>
      </Container>
    );
