...
const signUp = async params => {
  const { firstname, email, password, mount, profileimgurl } = params;
...
const userPhotoImpression = async params => {
  ...
  const { userId, photoId, liked, category } = params;
...
const addPhotoToDatabase = async params => {
  ...
  const query = `INSERT into photos (unsplashId, photographerName, downloadUrl, profileUrl, profileImageUrl, regularUrl, smallUrl, thumbUrl, category) Values ('${unsplashId}', '${photographerName}', '${downloadUrl}', '${profileUrl}', '${profileImageUrl}', '${regularUrl}', '${smallUrl}', '${thumbUrl}', '${category}');`;
  try {
...
const updateProfile = async params => {
  ...
  const { userId, firstName, email, mount, profileimgurl } = params;
  const about = parseStringSQL(params.about);
...
const addPhotoToDatabaseBeta = async params => {
  ...
  const query = `INSERT into "photos" (unsplashid, photographerName, profileUrl, profileImageUrl, regularUrl, smallUrl, thumbUrl, category)
  Values ('${unsplashId}', '${photographerName}', '${profileUrl}', '${profileImageUrl}', '${regularUrl}', '${smallUrl}', '${thumbUrl}', '${category}');`;
  console.log(query);
  try {
    const savePhoto = await client.query(query);
    console.log("Added Photo Id:", unsplashId, " to Postgres!");
...
const getPhotosFromIndex = async params => {
  const { category, amount, index } = params;
