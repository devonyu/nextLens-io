  ...
  try {
    ...
    Object.keys(formatted).forEach(categoryId => {
      formatted[categoryId] =
        formatted[categoryId].liked - formatted[categoryId].disliked;
    });
    // Sort Category IDs by formated scores
    const sorttedArray = await Object.keys(formatted).sort(
      (a, b) => formatted[b] - formatted[a]
    );
    ...
    await sorttedArray.forEach((category, idx) => {
      ...
      if (idx <= 2) {
        results[idx] =
          lensApi[brandName][mountId][categoriesAPI[category]].low[0];
        results[idx].category = category;
        results[idx + 3] =
          lensApi[brandName][mountId][categoriesAPI[category]].high[0];
        results[idx + 3].category = category;
      }
    });
    return results;
  } catch (err) {
