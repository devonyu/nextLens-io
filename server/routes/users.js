...
router
  ...
  .get("/:id/:mount/recommendations", async (req, res) => {
    ...
    try {
      ...
      const recommendationResults = await recommendationGenerator(
        allPhotoAffinities,
        mountId
      );
  ...
  .post("/:id/:category/:photoid", async (req, res) => {
    ...
    console.log(
      `Photo Impression Recieved=> UserID:${req.params.id}, Category:${req.params.category}, PhotoId:${req.params.photoid}, Affinity:${req.body.liked}`
    );
